    ...
    IPointerEnterHandler
{
    ...
    public void JumpTo(int target)
    {
        ...
        jumped = true;
        contentTarget.anchoredPosition = new Vector2(0, target * (heightText * 2));
    }

    public void JumpToLerp(int target)
    {
        if (target == currentCenter)
        {
            return;
        ...
        jumped = true;
        contentTarget.DOComplete();
        contentTarget.DOAnchorPosY(target * (heightText * 2), .5f);
    }
    /// <summary>
        ...
        if (isInitialized)
        {
            ...
            if (isInfinite)
            {
                if (contentTarget.anchoredPosition.y < middle)
                {
                    ...
                    {
                        contentTarget.GetChild(i).GetChild(0).GetComponent<Text>().fontSize = 0;
                    }
            ...
            var cachedCurrentCenter = Mathf.Clamp(Mathf.RoundToInt(contentPos / (heightText * 2)), 0,
                contentTarget.childCount - 1);


            if(currentCenter != cachedCurrentCenter)
            {
                currentCenter = cachedCurrentCenter;
                if (OnValueChanged != null && !jumped)
                {
                    OnValueChanged(cachedCurrentCenter);
                }
                jumped = false;
            }
            ...
            if (maxID > minID)
            {
                for (int i = minID; i < maxID; i++)
                {
                    ...
                    var currentText = contentTarget.transform.GetChild(i).GetChild(0).GetComponent<Text>();
                    float ratio =
                        Mathf.Clamp(
                    ...
                    currentText.fontSize = (int) (maxFontSize * curve.Evaluate(ratio));
